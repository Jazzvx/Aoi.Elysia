...
code: `$title[Hello!] $description[> You added me and don't know how to use it? Nothing, below this question.] $color[RANDOM] $addField[Help! How to use a bot?;To see a list of all commands, you need to write $getserverVar[prefix]help] 
$suppressErrors[Error!
> Support Server : https://discord.gg/87mSxdzhJ4 ]`
})
bot.onGuildJoin() 

//Pinged
bot.command({
  name: '<@894526635464876042>',
  code: `Hello $username My prefix on this server is $getServerVar[prefix]`,
  nonPrefixed: true
})

bot.userUpdateCommand({ 
name: "status logger",
channel: "1028942611089723402", 
code: `
$title[$customEmoji[$oldUser[status]_status] | Uptime Alert]
$description[$replaceText[$replaceText[$oldUser[status];online;Looks like <@$oldUser[id]> is back Online.];offline;Looks like <@$oldUser[id]> is currently down.]]
$color[$replaceText[$replaceText[$oldUser[status];online;GREEN];offline;GREY]]
$thumbnail[$oldUser[avatar]]
$footer[Uptime alert;$oldUser[avatar]]
$addTimestamp
$onlyIf[$checkContains[$oldUser[status];online;offline]==true;]
$onlyIf[$isBot[$oldUser[id]]==true;]
`
})

//Welcome Leave
//welcome with image, the bot may not send the image
...
$color[$getVar[color]]
$addTimestamp
$setGlobalUserVar[commanduserused;$sum[$getGlobalUserVar[commanduselrused];1]]
